...
//
Camera::Camera(void)
	:m_vEye(D3DXVECTOR3(5, 5, -15)), m_vLookAt(D3DXVECTOR3(0, 0, 0)),
	m_vUp(D3DXVECTOR3(0, 1, 0)), m_vViewDir(m_vLookAt - m_vEye),
	m_vOrgEye(m_vEye), m_vOrgLookAt(m_vLookAt),
	m_fFov(D3DXToRadian(45)), m_fAspect((float)WINDOWMGR->GetWidth() / WINDOWMGR->GetHeight()),
	m_fZnear(1.0f),
	m_fZfar(1000.0f), m_bCameraAutoRotate(false)	
{
...
:m_vEye(_vEye),m_vLookAt(_vLookAt),
m_vUp(_vUp),m_fFov(_Fov),
m_fAspect(_Aspect),m_fZnear(_near),
m_fZfar(_far), m_bCameraAutoRotate(false)
{
	D3DXMatrixIdentity(&m_mView);
	D3DXMatrixIdentity(&m_mInvView);
	D3DXMatrixIdentity(&m_mProj);
